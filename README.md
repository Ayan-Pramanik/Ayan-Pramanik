# Hi, I'm Ayan 👋

🎓 Master's student in Cybersecurity
🛡️ Aspiring SOC / Cybersecurity Analyst
💻 Software Development + Security Automation

## 🔭 Currently Working On

- SOC & Blue Team operations
- SIEM and detection engineering
- Windows & Active Directory security
- Threat hunting
- Incident response
- Python security automation

## 🛠️ Security Skills

### Security
SOC | SIEM | Incident Response | Threat Hunting
MITRE ATT&CK | Digital Forensics | Network Security

### Systems
Linux | Windows | Active Directory

### Tools
Wireshark | Nmap | Splunk | Elastic | Sentinel
Burp Suite | Sysmon | Kali Linux

### Programming
Python | JavaScript | TypeScript

## 🚀 Featured Projects

- Home SOC Lab
- Threat Hunting Laboratory
- Incident Response Case Study
- Network Security Monitoring
- Digital Forensics Investigation
- IOC Investigation Toolkit

## 📚 Currently Learning

- Active Directory Security
- Detection Engineering
- Azure Security
- CCNA
- Security Automation

## 📫 Connect

LinkedIn: ...
